...
async def build_application(args: argparse.Namespace) -> dict:
    ...
    if config.get("browser.cleanup_orphans_on_start", True):
    ...
    if Path(proxy_file).exists():
        loaded = await proxy_engine.load_from_file(
            proxy_file,
            validate=config.get("proxy.validate_on_load", False),
        )
        logger.info(f"Loaded {loaded} proxies")
    elif config.get("proxy.enabled", True) and config.get("proxy.required", True):
        logger.error(
            f"Proxy file not found: {proxy_file}. Strict proxy mode is enabled. "
            "Campaign sessions will not launch without a proxy."
        )
    if (
        ...
        logger.warning(
            "Strict proxy mode is enabled but no proxies were loaded. "
            "This prevents real-IP browser launches."
        )
    ...
    browser_farm = BrowserFarm(
        ...
        max_memory_mb   = config.get("browser.max_memory_mb", 4096.0),
    ...
    orchestrator = TrafficOrchestrator(
        ...
        event_bus           = bus,
        max_concurrent      = config.get("general.max_workers", 1),
    )
...
async def run_campaign(components: dict, args: argparse.Namespace) -> None:
    """Create and run a traffic campaign."""
    config = components["config"]
    orchestrator = components["traffic_orchestrator"]
    target_urls = (
        args.targets.split(",") if args.targets else config.get("traffic.target_urls", [])
    )
    if not target_urls:
...
async def shutdown(components: dict) -> None:
    ...
    if config:
        ...
        if config.get("browser.cleanup_orphans_on_stop", True):
            try:
                from engines.browser.browser_controller import BrowserInstance
                BrowserInstance.cleanup_orphaned_chrome_processes()
            except Exception as exc:
                logger.debug(f"Chrome orphan cleanup skipped: {exc}")
